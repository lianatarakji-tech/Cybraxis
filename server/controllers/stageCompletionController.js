...
} = require("../services/stages/stageCompletionEngine");
...
async function evaluateStageCompletionForSession(req, res) {
  ...
  const backendStageCompletion = evaluateStageCompletion({
  ...
  const stageCompletionParity = compareStageCompletion({
    frontendStageCompletion,
    backendStageCompletion,
  });

  if (!stageCompletionParity.matches) {
    console.warn("BACKEND STAGE COMPLETION PARITY MISMATCH", {
      sessionId,
      stageId: stageForEvaluation.id,
      stageCompletionParity,
    });
  }
